...
const BOT_NAME_FANCY = '𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐕3';
...
const config = {
  ...
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/x57uwq.png' }
};
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    let botName = cfg.botName || '𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐕3';
...
case 'gpt': {
  try {
    ...
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐕3 𝐀ɪ';
 ...
 case 'weather':
    try {
        ...
        const messages = {
            ...
            weather: (data) => `
*💝 𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐕3 𝐖ᴇᴀᴛʜᴇʀ 𝐑ᴇᴘᴏʀᴛ*
...
> 𝐏ᴏᴡᴇʀᴇᴅ 𝐁ʏ 𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐎ᴡɴᴇʀ 𝐀ɴᴜɢᴀ 𝐒ᴇɴɪᴛʜᴜ
`,
...
case 'apk': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'QUEEN 94710695082 MINI BOT'
...
END:VCARD`
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'ANUWH MINI BOT AI';
...
_© Powered by ${botName}_`;
 ...
 case 'video': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐕3';    // fake contact for quoted card
    const botMention = {
        key: {
            remoteJid: "status@broadcast",
            participant: "0@s.whatsapp.net",
...
case 'tourl': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '> 𝐏ᴏᴡᴇʀᴇᴅ 𝐁ʏ 𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐎ᴡɴᴇʀ 𝐀ɴᴜɢᴀ 𝐒ᴇɴɪᴛʜᴜ' // fake contact for quoted card
    const botMention = {
        key: {
            remoteJid: "status@broadcast",
            participant: "0@s.whatsapp.net",
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '> 𝐏ᴏᴡᴇʀᴇᴅ 𝐁ʏ 𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐎ᴡɴᴇʀ 𝐀ɴᴜɢᴀ 𝐒ᴇɴɪᴛʜᴜ';
...
> © ${config.BOT_FOOTER || '> 𝐏ᴏᴡᴇʀᴇᴅ 𝐁ʏ 𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐎ᴡɴᴇʀ 𝐀ɴᴜɢᴀ 𝐒ᴇɴɪᴛʜᴜ'}
`.trim();
    ...
    const defaultImg = 'https://files.catbox.moe/x57uwq.png';
    ...
    await socket.sendMessage(sender, {
      image: imagePayload,
      caption: text,
      footer: "> 𝐏ᴏᴡᴇʀᴇᴅ 𝐁ʏ 𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐎ᴡɴᴇʀ 𝐀ɴᴜɢᴀ 𝐒ᴇɴɪᴛʜᴜ",
      buttons,
      headerType: 4
    }, { quoted: shonux });
...
case 'download': {
  ...
  try {
    ...
    const title = userCfg.botName || 'ANUWH MINI';
...
case 'creative': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'ANUWH MINI BOT AI';
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> 𝐏ᴏᴡᴇʀᴇᴅ 𝐁ʏ 𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐎ᴡɴᴇʀ 𝐀ɴᴜɢᴀ 𝐒ᴇɴɪᴛʜᴜ',
            buttons: buttons,
            headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
case 'owner': {
  try {
    ...
    let vcard = 
      ...
      'TITLE:Founder & CEO of Anuwh MD Mini Bot;\n' + // Title / Role
      'EMAIL;type=INTERNET:anuga.senithu2013official@gmail.com\n' + // Email
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'ANUWH MINI BOT AI'; // dynamic bot name
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname ANUWH MINI - 01`' }, { quoted: shonux });
  }
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || 'ANUWH-MINI-main'}`); } catch(e) { console.error('pm2 restart failed', e); }
        }
      }
      if (connection === 'close') {
        try { if (fs.existsSync(sessionPath)) fs.removeSync(sessionPath); } catch(e){}
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '𝐀ɴᴜᴡʜ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ 𝐕3', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || 'ANUWH-MINI-main'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
